...
export function UserProfile() {
  const { currentUser, userProfile, signInWithGoogle, logout, updateUsername } =
    useAuth();
  ...
  if (!currentUser) {
    return (
      <div className="frosted-glass p-6 w-72 flex flex-col items-center gap-4 border-[rgba(255,255,255,0.1)]">
        ...
        <h3 className="font-futuristic-header text-lg text-white">
          Guest Operator
        </h3>
        <p className="text-white/50 text-sm text-center font-mono-code mb-2">
          Sign in to access global network leaderboards and store your BPS
          rating.
        </p>
        <button
          onClick={(e) => {
  ...
  return (
    <div
      className="frosted-glass p-6 w-72 flex flex-col items-center gap-4 border-[rgba(0,229,255,0.2)] shadow-[0_0_15px_rgba(0,229,255,0.1)]"
    ...
    >
      <div className="relative group">
        {userProfile?.photoURL ? (
          <img
            ...
            alt="Profile"
            className="w-16 h-16 rounded-full border-2 border-[var(--color-accent-cyan)] object-cover shadow-[0_0_10px_var(--color-accent-cyan)]"
          />
        ) : (
          ...
          >
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-1 text-xs font-futuristic-header text-white/50 hover:text-white cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="flex-1 py-1 text-xs font-futuristic-header text-[var(--color-accent-magenta)] hover:text-white cursor-pointer"
              >
                Save
              </button>
            </div>
      ...
      <div className="w-full flex justify-between items-center px-2">
        <span className="text-white/50 text-xs uppercase tracking-widest font-futuristic-header">
          Top Speed
        </span>
